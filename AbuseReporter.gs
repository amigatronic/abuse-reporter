/**
 * ============================================================
 * ABUSE REPORTER - Final Hardened Version
 * ============================================================
 */
...
function processOneMessage(thread, message, myPublicIp, sentToProvider, reviewLabel, fpLabel) {
  ...
  var evalResult = evaluateMessage(rawHeader, bodyText, subject, fromRaw);
  logToSheet([new Date(), evalResult.category, evalResult.score, subject, fromRaw]);
...
function cleanIp(ip) {
  if (!ip) return "";
  // Remove brackets, parentheses, quotes, and trailing zone IDs (e.g., %eth0)
...
function isSafeAbuseTarget(ip, abuseEmails, senderDomain) {
  ...
  var suspicious = abuseEmails.every(function(email) {
    var domain = email.split("@")[1];
    // Use endsWith to avoid false negatives (e.g., "notexample.com" contains "example.com")
  ...
  bootstrapData.services.forEach(function(service) {
    ...
    prefixes.forEach(function(prefix) {
      ...
      if (!isV6) {
        ...
        if (ipNorm === prefixNorm || ipNorm.startsWith(prefixNorm + ":")) {
          if (mask > bestMask) {
            bestMask = mask;
            bestUrl = urls[0];
          }
...
function queryIpApi(ipAddress) {
  ...
  // 2. Fallback to ip-api.com (HTTP only for free tier)
